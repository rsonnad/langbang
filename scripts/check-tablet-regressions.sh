...
require_fixed "$verbs" "label = \"random\"" "Random queue checkbox in the verbs filter row"
...
require_fixed "$verbs" "commonObjectNounsForVerb" "broader common noun pools for local phrases"
require_fixed "$verbs" "spaceLocalPhraseCandidates" "balanced local phrase vocabulary spacing"
require_absent_fixed "$verbs" "\"mieć\", \"widzieć\", \"lubić\", \"kochać\" -> listOf(\"dom\", \"kawa\", \"książka\", \"telefon\", \"samochód\")" "tiny repetitive generic phrase noun pool"
require_fixed "$verbs" "showPhraseGenerationNotice()" "phrase-generation ETA notice"
require_fixed "$verbs" "quickTargetPlayCount(allVerbs)" "immediate phrase play count preview"
require_fixed "$verbs" "private fun selectedSentences(" "phrase play/preview draws from the filtered natural-sentence pool (LLM-first, no local assembler)"
require_fixed "$verbs" "val exactCount = state.targetPlayCount(allVerbs)" "computed phrase play count preview"
require_fixed "$verbs" "playLabel = if (playCountPreview == null) \"Play ...\" else null" "no overpromised phrase play count"
...
require_fixed "$word_header" ".width(54.dp)" "compact vars stepper width"
require_fixed "$word_header" ".width(17.dp)" "compact vars stepper arrows"
...
require_fixed "app/src/main/kotlin/com/sponic/langbang/ui/common/NowVoicingPanel.kt" "Icons.Default.Repeat" "Now Voicing Loop icon toggle"
require_fixed "app/src/main/kotlin/com/sponic/langbang/ui/common/StudyQueuePlayer.kt" "loopPractice()) 4 else 1" "shared queue four-pass loop"
...
require_fixed "$nouns" "fun playPolishOnce(text: String)" "Nouns tapped playback synthesizes before play"
require_fixed "$nouns" "buildCheckedPlaybackQueue" "Nouns Play queue falls back to forms"
require_fixed "$nouns" "pool.isEmpty() -> nounFormItems(noun, limit)" "Nouns Play form fallback when examples are missing"
require_absent_fixed "$nouns" "app.audioCache.fileFor(" "raw Nouns audio path without cache/synth"
...
require_fixed "$prefs" "loopPractice()" "Now Voicing loop practice preference"
...
require_fixed "$gemini" "const val SENTENCE_PROMPT_VERSION = 7" "tagged (cat/person) R2 sentence bundle version w/ object pronouns"
require_fixed "$gemini" "const val VERB_WIPE_VERSION = 6" "verb cache wipe for object-pronoun (v7) sentence bundles"
